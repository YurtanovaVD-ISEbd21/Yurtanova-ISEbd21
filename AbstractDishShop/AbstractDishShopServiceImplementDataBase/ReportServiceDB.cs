    ...
    public class ReportServiceDB : IReportService
    {
        ...
        public void SaveDishPrice(ReportBindingModel model)
        {
            ...
            try
            {
                ...
                var Dishs = context.Dishs.ToList();
               
            //создаем таблицу
var paragraphTable = document.Paragraphs.Add(Type.Missing);
                var rangeTable = paragraphTable.Range;
                var table = document.Tables.Add(rangeTable, Dishs.Count, 2, ref missing, ref missing);
                ...
                {
                    table.Cell(i + 1, 1).Range.Text = Dishs[i].DishName;
                    table.Cell(i + 1, 2).Range.Text = Dishs[i].Price.ToString();
                }
        ...
        public List<StocksLoadViewModel> GetStocksLoad()
        {
            ...
            context.StockMaterialss
            .Include(r => r.Materials)
            .ToList(),
            stock => stock,
            stockMaterials => stockMaterials.Stock,
            (stock, stockCompList) =>
            new StocksLoadViewModel
            {
                StockName = stock.StockName,
                TotalCount = stockCompList.Sum(r => r.Count),
                Materialss = stockCompList.Select(r => new Tuple<string, int>(r.Materials.MaterialsName, r.Count))
            })
            .ToList();
        }
...
excel.Workbooks[1].Save();
        ...
        public List<SClientSOrdersModel> GetSClientSOrders(ReportBindingModel model)
        {
            return context.SOrders
            .Include(rec => rec.SClient)
            .Include(rec => rec.Dish)
            .Where(rec => rec.DateCreate >= model.DateFrom && rec.DateCreate <= model.DateTo)
            .Select(rec => new SClientSOrdersModel
            {
        ...
        public void SaveSClientSOrders(ReportBindingModel model)
        {
            ...
            //заполняем таблицу
            var list = GetSClientSOrders(model);
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].DishName, fontForCells));
                table.AddCell(cell);
